...
{  $myusername = mysqli_real_escape_string($con,$_POST["login_string"]);
	   ...
	   $sql = "SELECT id FROM mydata WHERE `email` = '$myusername' && `password` = '$mypassword'";
	   echo $sql;
...
{
 $_SESSION['login_user'] = $_POST["login_string"] ;
 header("location: userdashboard.php");
     echo "successful";
}
}else {
...
<br> 
                  ...
                  <div class="text-center">
                    <a class="small" href="adminlogin.php">Login as Administrator</a>
                  </div>
                </div>
              </div>
            </div>
